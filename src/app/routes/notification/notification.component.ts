...
export class NotificationsComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  async getAccessToken() {
    // Access/refresh tokens are no longer persisted in localStorage (only login status is).
    // The notification socket is currently disabled; if it's revived, source the token
    // from the auth service (e.g. KeycloakService.getToken()) rather than browser storage.
    return ''
  }
