...
export class SignupService {
  ...
  private applyStartUpDetailsToConfig(details: any): void {
  ...
  private findOrgMatchForLoggedInUser(states: any[]): any {
  ...
  private findOrgMatchFromUrlParam(states: any[]): any {
  ...
  private saveMatchedOrgSelectiveConfig(matchedOrg: any, states: any[]): void {
    if (matchedOrg) {
  ...
  private async fetchOrgSelectiveConfig(): Promise<void> {
    try {
      ...
      const { states } = orgSelectiveData
      // 1. Try matching for logged-in user (rootOrgId); 2. fall back to ?org= param (public route)
      const matchedOrg = this.findOrgMatchForLoggedInUser(states) || this.findOrgMatchFromUrlParam(states)

      // 🔹 3. Save matched config
      this.saveMatchedOrgSelectiveConfig(matchedOrg, states)
    } catch (error) {
